...
**Nom de domaine cible : `www.pizza-vellano.fr`.** Un fichier `CNAME`
contenant `www.pizza-vellano.fr` est présent à la racine du dépôt : une
fois la zone DNS du domaine pointée vers GitHub Pages (CNAME vers
`sachaprovence.github.io`) et le domaine personnalisé activé dans les
paramètres Pages du dépôt, le site sera servi directement sur ce nom de
domaine. Les balises `canonical`, Open Graph et les données structurées
Schema.org du site pointent déjà vers ce domaine.
